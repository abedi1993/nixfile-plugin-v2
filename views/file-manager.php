...
final class NixfileSettings {
    public readonly string $token;
    public readonly bool $showStatusNavbar;
    public readonly bool $compressUpload;
    public readonly bool $avifOnUpload;
    public readonly bool $jalaliConverter;
    public readonly bool $modernTemplate;
    public readonly bool $externalFeaturedImage;
    public readonly string $defaultExternalImage;
    public readonly bool $dailyBackup;

    public function __construct() {
        $this->token = get_option(NixfileSetting::TOKEN->value, '');
        $this->showStatusNavbar = (bool) get_option(NixfileSetting::SHOW_STATUS_NAVBAR->value, false);
        $this->compressUpload = (bool) get_option(NixfileSetting::COMPRESS_UPLOAD->value, false);
        $this->avifOnUpload = (bool) get_option(NixfileSetting::AVIF_ON_UPLOAD->value, false);
        $this->jalaliConverter = (bool) get_option(NixfileSetting::JALALI_CONVERTER->value, false);
        $this->modernTemplate = (bool) get_option(NixfileSetting::MODERN_TEMPLATE->value, false);
        $this->externalFeaturedImage = (bool) get_option(NixfileSetting::EXTERNAL_FEATURED_IMAGE->value, false);
        $this->defaultExternalImage = get_option(NixfileSetting::DEFAULT_EXTERNAL_IMAGE->value, 'https://bostak1337.ir/wp-content/uploads/2025/10/shakes-image.webp');
        $this->dailyBackup = (bool) get_option(NixfileSetting::DAILY_BACKUP->value, false);
    }
}

$settings = new NixfileSettings();
?>
<div class="nixfile-container">
    ...
    <header class="nixfile-media-header">
        <h1>کتابخانه رسانه نیکس فایل</h1>
        <div>
            <button id="nixfile-uploader-opener">افزودن رسانه جدید</button>
            <button id="nixfile-folder-opener">افزودن پوشه جدید</button>
        </div>
    </header>
    ...
    <label id="nixfile-uploader" class="nixfile-uploader">
        ...
        <div class="nixfile-uploader-content">
            ...
            <p>یا</p>
            <span class="nixfile-button">گزینش پرونده‌ها</span>
            <input type="file" multiple>
            <p>حداکثر اندازه پرونده برای بارگذاری: 8 مگابایت.</p>
        </div>
    </label>
    ...
    <div class="nixfile-setting">
        ...
        <div class="nixfile-token">
            <label>
                ...
                <input placeholder="توکن" value="<?= esc_attr($settings->token) ?>" name="nixfile_store_token" type="text">
            </label>
            <p>
                <span>شمسی ساز تاریخ‌ها</span>
                <button id="jalali-converter" style="<?= $settings->jalaliConverter ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->jalaliConverter ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
            <p>
                <span>فعال سازی قالب مدرن</span>
                <button id="modern-template" style="<?= $settings->modernTemplate ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->modernTemplate ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
        </div>
        <div class="nixfile-option">
            <p>
                ...
                <button id="daily-backup" style="<?= $settings->dailyBackup ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->dailyBackup ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
            <p>
                <span>نمایش مانیتورینگ در نوار وردپرس </span>
                <button id="nixfile-show-on-navbar" style="<?= $settings->showStatusNavbar ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->showStatusNavbar ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
            <p>
                <span>تصویر شاخص خارجی</span>
                <button id="external-featured-image" style="<?= $settings->externalFeaturedImage ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->externalFeaturedImage ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
        </div>
        <div class="nixfile-option">
            <p>
                ...
                <button id="compares-on-upload" style="<?= $settings->compressUpload ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->compressUpload ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
            <p>
                <span>زمان آپلود فرمت AVIF شود؟</span>
                <button id="avif-on-upload" style="<?= $settings->avifOnUpload ? "background-color:#00aa2c" : "background-color:red;" ?>">
                    <?= $settings->avifOnUpload ? "فعال" : "غیر فعال" ?>
                </button>
            </p>
            <p class="default-external-image-container" style="<?= $settings->externalFeaturedImage ? 'display: flex;' : 'display: none;' ?>">
                <span>آدرس تصویر پیش‌فرض</span>
                <input type="text" id="default-external-image" placeholder="https://example.com/default-image.jpg" value="<?= esc_attr($settings->defaultExternalImage) ?>" style="width: 100%; margin-right: 10px;">
    ...
    <div class="nixfile-folder-form-container">
        <form class="nixfile-folder-form" method="POST">
            <img src="<?= plugin_dir_url(__DIR__) . 'assets/images/add.svg' ?>" alt="">
            <label>
                <span>نام پوشه</span>
        ...
        <ul>
            <li id="nixfile-edit-folder">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="m10 21l4-4h8v4H10Zm-6-2h1.4l8.625-8.625l-1.4-1.4L4 17.6V19ZM184 9L270.603 159H97.3975L184 9ZM541 0L627.603 150H454.397L541 0Z"/>
                </svg>
                <span>ویرایش نام</span>
            </li>
            <li id="nixfile-delete-folder">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="M7.616 20q-.672 0-1.144-.472T6 18.385V6H5V5h4v-.77h6V5h4v1h-1v12.385q0 .69-.462 1.153T16.384 20H7.616ZM17 6H7v12.385q0 .269.173.442t.443.173h8.769q.23 0 .423-.192t.192-.424V6ZM9.808 17h1V8h-1zm3.384 0h1V8h-1zM7 6v13z"/>
                </svg>
                <span>حـذف پوشه</span>
            </li>
            <li id="nixfile-move-folder">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="m7 21l-1.4-1.4l1.575-1.65q-2.65-.3-4.413-2.287T1 11q0-2.925 2.038-4.962T8 4h3v2H8Q5.925 6 4.463 7.463T3 11q0 1.8 1.15 3.175T7.075 15.9L5.6 14.425L7 13l4 4zm6-1v-7h9v7zm0-9V4h9v7zm2-2h5V6h-5z"/>
                </svg>
                <span>انتقال پوشه</span>
            </li>
            <li id="nixfile-detail-folder">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="M8 12v-2h8v2zm0-4V6h8v2zm-2 6h8.975L18 17.95V4H6zm0 6h11.05L14 16H6zm14 2H4V2h16zM6 20V4zm0-4v-2z"/>
                </svg>
    ...
    <div class="nixfile-file-contextmenu">
        <ul>
            <li id="nixfile-copy-file">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="M9 18q-.825 0-1.412-.587T7 16V4q0-.825.588-1.412T9 2h9q.825 0 1.413.588T20 4v12q0 .825-.587 1.413T18 18zm0-2h9V4H9zm-4 6q-.825 0-1.412-.587T3 20V6h2v14h11v2zm4-6V4z"/>
                </svg>
                <span>کپی لینک</span>
            </li>
            <li id="nixfile-edit-file-name">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="M5 19h1.425L16.2 9.225L14.775 7.8L5 17.575zm-2 2v-4.25L16.2 3.575q.3-.275.663-.425t.762-.15t.775.15t.65.45L20.425 5q.3.275.438.65T21 6.4q0 .4-.137.763t-.438.662L7.25 21zM19 6.4L17.6 5zm-3.525 2.125l-.7-.725L16.2 9.225z"/>
                </svg>
                <span>ویرایش نام</span>
            </li>
            ...
            <li id="nixfile-move-file">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                    <path fill="#888888" d="m12.2 14l-1.625 1.625l1.4 1.4L16 13l-4.025-4.025l-1.4 1.4L12.2 12H8v2zM4 20q-.825 0-1.412-.587T2 18V6q0-.825.588-1.412T4 4h6l2 2h8q.825 0 1.413.588T22 8v10q0 .825-.587 1.413T20 20zm0-2h16V8h-8.825l-2-2H4zm0 0V6z"/>
                </svg>
            ...
            <li id="nixfile-detail-file">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 36 36">
                    <path fill="#888888" d="M25.39 25.45a1 1 0 0 0-1.38.29c-1.41 2.16-4 4.81-6.31 5.7s-4.12.57-4.84 0c-.31-.27-1.12-1-.43-3.49c.46-1.66 3.32-9.48 4-11.38l-2.18.28c-.69 1.86-3.29 8.84-3.76 10.58c-.68 2.49-.34 4.3 1.09 5.56A5.6 5.6 0 0 0 15 34a9.5 9.5 0 0 0 3.45-.7c2.79-1.09 5.72-4.12 7.26-6.47a1 1 0 0 0-.32-1.38" class="clr-i-outline clr-i-outline-path-1"/>
                    <path fill="#888888" d="M19.3 11a4.5 4.5 0 1 0-4.5-4.5a4.5 4.5 0 0 0 4.5 4.5m0-7a2.5 2.5 0 1 1-2.5 2.5A2.5 2.5 0 0 1 19.3 4" class="clr-i-outline clr-i-outline-path-2"/>
                    <path fill="#888888" d="M11.81 15c.06 0 6.27-.82 7.73-1c.65-.1 1.14 0 1.3.15s.21.8-.07 1.68c-.61 1.86-3.69 11-4.59 13.71a8 8 0 0 0 1.29-.38a7.3 7.3 0 0 0 1.15-.6c1.23-3.56 3.53-10.46 4.05-12.04s.39-2.78-.3-3.6a3.16 3.16 0 0 0-3.08-.83c-1.43.15-7.47.94-7.73 1a1 1 0 0 0 .26 2Z" class="clr-i-outline clr-i-outline-path-3"/>
    ...
    <div id="nixfile-folder-move-container">
        <div class="nixfile-folder-move-content">
            <div class="nixfile-folder-mov-header">
                <img class="nixfile-size-12" src="<?= plugin_dir_url(__DIR__) . 'assets/images/transfer.svg' ?>" alt="alt">
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const elements = {
            jalaliConverterBtn: document.getElementById('jalali-converter'),
            modernTemplateBtn: document.getElementById('modern-template'),
            externalFeaturedImageBtn: document.getElementById('external-featured-image'),
            defaultExternalImageInput: document.getElementById('default-external-image'),
            saveDefaultExternalImageBtn: document.getElementById('save-default-external-image'),
            defaultExternalImageContainer: document.querySelector('.default-external-image-container'),
            dailyBackupBtn: document.getElementById('daily-backup')
        };

        const toggleButton = (btn, enabled) => {
            btn.textContent = enabled ? 'فعال' : 'غیر فعال';
            btn.style.backgroundColor = enabled ? '#00aa2c' : 'red';
            return enabled;
        };
        ...
        const makeApiCall = async (endpoint, data) => {
            ...
            try {
                const response = await fetch(nixfile_ajax_data.rest_url + endpoint, options);
                const result = await response.json();

                if (result.success) {
                    console.log(`${endpoint} setting updated successfully`);
                    return result;
                } else {
                    console.error(`Failed to update ${endpoint} setting`);
                    return null;
                }
            } catch (error) {
                console.error(`Error updating ${endpoint} setting:`, error);
                return null;
            }
        };

        elements.jalaliConverterBtn?.addEventListener('click', function() {
            const newState = !this.textContent.includes('فعال');
            toggleButton(this, newState);
            makeApiCall('jalali-converter', { jalali_converter: newState })
                .then(() => location.reload());
        });

        elements.modernTemplateBtn?.addEventListener('click', function() {
            const newState = !this.textContent.includes('فعال');
            toggleButton(this, newState);
            makeApiCall('modern-template', { modern_template: newState })
                .then(() => location.reload());
        });

        elements.externalFeaturedImageBtn?.addEventListener('click', function() {
            const newState = !this.textContent.includes('فعال');
            toggleButton(this, newState);
            makeApiCall('external-featured-image', { external_featured_image: newState })
                .then(() => {
                    elements.defaultExternalImageContainer.style.display = newState ? 'flex' : 'none';
                    location.reload();
                });
        });

        elements.dailyBackupBtn?.addEventListener('click', function() {
            const newState = !this.textContent.includes('فعال');
            toggleButton(this, newState);
            makeApiCall('daily-backup', { daily_backup: newState })
                .then(result => {
                    if (result?.data?.immediate_backup) {
        ...
        elements.saveDefaultExternalImageBtn?.addEventListener('click', () => {
            const imageUrl = elements.defaultExternalImageInput.value.trim();
            if (!imageUrl) {
                alert('لطفاً یک آدرس تصویر معتبر وارد کنید');
                return;
            }

            makeApiCall('settings', { default_external_image: imageUrl })
                .then(result => {
                    if (result) {
